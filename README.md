...
### 👨‍👩‍👧 親の声ジェネレーター (Custom Features)

本プロジェクトは「親の声ジェネレーター」として、以下のカスタム機能が組み込まれています：
- **Irodori-TTS連携**: クロスオリジン対応のFastAPIバックエンドによる高品質な音声合成・クローニング機能。
- **Gemini 3.1 Pro 連携**: Gemini 3.1 Pro (High)モデルを活用した、優しく見守る「親」ペルソナでの対話パイプライン。
- **モーション・リップシンク最適化**: より自然な動きを実現するためのトラッキング・ポーズソルバーの独自調整。
