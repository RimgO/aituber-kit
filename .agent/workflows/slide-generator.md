...
3.  **Generate scripts.json**: Parse the provided script text and create `public/slides/[Slide Name]/scripts.json`.
    - _Note_: You will need to manually map the script lines to the slide pages. By default, split the script by newlines or paragraphs and assign them sequentially to `page: 0`, `page: 1`, etc.
    - **Format**:
      ```json
      [
        {
